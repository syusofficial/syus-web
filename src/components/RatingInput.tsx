...
export default function RatingInput({ showId, isLoggedIn, initialMyScore }: Props) {
  ...
  if (!isLoggedIn) {
    return (
      <div className="text-sm" style={{ fontFamily: "var(--font-noto-sans-kr)", color: "#6B7385" }}>
        별점을 부여하시려면{" "}
        <Link
          href="/auth/login"
          className="underline"
          style={{ color: "#274E9B", fontWeight: 600 }}
        >
          로그인
        </Link>
        해주세요.
...
function Star({ size = 24, filled = false }: { size?: number; filled?: boolean }) {
  return (
    <svg
      ...
      height={size}
      viewBox="0 0 24 24"
      fill={filled ? "#F5C84F" : "none"}
      stroke={filled ? "#F5C84F" : "#C5CCD9"}
      strokeWidth="1.5"
