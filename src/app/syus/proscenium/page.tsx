...
export const metadata: Metadata = {
  ...
  description: "운영자가 연기를 오래 들여다본 글 '연기와 냉장고'. 시우스는 연기 커뮤니티입니다.",
...
export default async function ProsceniumHub() {
  ...
  return (
    <main className="syc-wrap" style={{ ["--c" as string]: COLOR } as React.CSSProperties}>
      ...
      <p className="syc-lead">운영자가 연기를 오래 들여다본 글을 한 편씩 액자에 걸듯 내겁니다. ‘연기와 냉장고’ — 차곡차곡 재워둔 관찰입니다.</p>
          ...
          <div className="syc-cards">
            {essays.map((e) => (
              <Link key={e.id} href={`/syus/essays/${e.id}`} className="syc-card">
                {e.series_no != null && <span className="syc-card-meta">연기와 냉장고 · {String(e.series_no).padStart(2, "0")}</span>}
