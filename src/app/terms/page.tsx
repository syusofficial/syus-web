...
export default function TermsPage() {
  const sections = [
    ...
    },
    {
      ...
      body: (
        <ol className="list-decimal pl-5 space-y-1">
          <li><strong>창작 독백 기능은 생성형 인공지능을 기반으로 제공되는 서비스입니다.</strong> 회사는 「인공지능 발전과 신뢰 기반 조성 등에 관한 기본법」 제31조 제1항에 따라 이 사실을 회원이 이용하기 전에 고지하며, 같은 조 제2항에 따라 생성된 독백에는 인공지능이 생성한 결과물이라는 표시를 함께 제공합니다.</li>
          <li>창작 독백은 회원의 요청에 따라 인공지능(AI)이 새로 지어낸 대사이며, 요청 즉시 생성되어 별도의 운영자 검수 없이 요청 회원에게 곧바로 전달됩니다. 생성에 실패한 요청은 전달되지 않으며, 회원은 다시 요청하실 수 있습니다.</li>
          <li>회사는 전달 전에 독백 내용을 미리 검토하지 않습니다. 다만 전달 후 신고가 접수되거나 부적절한 내용이 확인되는 경우, 제12조·제13조에 따라 해당 독백을 비공개·삭제할 수 있습니다.</li>
    ...
    },
    {
      ...
      body: (
        <ol className="list-decimal pl-5 space-y-1">
          <li>회원 또는 비회원(이하 &ldquo;이용자&rdquo;)은 공연 상세 페이지의 좌석 신청 기능을 통해 무료 관람을 위한 좌석(참석)을 신청할 수 있습니다. 본 신청은 결제를 수반하지 않으며, 실제 입장은 공연팀·학교 현장 안내에 따릅니다.</li>
          <li>좌석 신청 취소는 해당 공연 시작 전까지 별도의 시간 제한 없이 가능하며, 취소로 인한 위약금 등 불이익은 없습니다.</li>
          <li>정원이 있는 공연에서 확정된 신청이 취소되는 경우, 대기 순번에 따라 자동으로 다음 신청이 확정 처리될 수 있습니다.</li>
          <li>회사는 신청 취소에 따른 자동 안내(이메일 등)를 발송할 수 있으며, 이용자가 신청 시 기재한 연락처의 정확성에 대해서는 이용자 본인이 책임집니다.</li>
          <li>공연팀이 좌석 신청 접수를 수동으로 마감한 경우, 이용자는 신규 신청 또는 대기 신청을 할 수 없습니다.</li>
          <li>정당한 사유 없이 반복적으로 신청 후 불참(노쇼)하는 이용자에 대해 회사는 개별 안내를 할 수 있으나, 이 조항만으로 회원자격을 제한하지는 않습니다.</li>
        </ol>
      ),
    },
    {
    ...
    },
    {
      ...
      body: (
        <ol className="list-decimal pl-5 space-y-1">
          ...
          <li>제18조(좌석 신청 및 취소) 신설분은 2026년 7월 14일부터 시행됩니다. 기존 제18조(분쟁 해결)는 제19조로 이동하였습니다.</li>
          <li>제17조 제1항·제2항(창작 독백의 생성·전달 방식) 정비분은 2026년 8월 3일부터 시행됩니다. 2026년 7월 29일부터 창작 독백은 운영자 검수 단계 없이 생성 즉시 전달되고 있으며, 본 개정은 그 실제 운영 방식을 약관 문구에 맞춘 것입니다. 이에 따라 기존 제17조 제2항부터 제6항까지는 각 한 항씩 뒤로 이동하였습니다.</li>
          <li>제17조 제1항(생성형 인공지능 기반 서비스 고지) 신설분은 2026년 9월 18일부터 시행됩니다. 「인공지능 발전과 신뢰 기반 조성 등에 관한 기본법」(2026년 1월 22일 시행) 제31조의 고지·표시 의무를 약관에 반영한 것이며, 이에 따라 기존 제17조 제1항부터 제7항까지는 각 한 항씩 뒤로 이동하였습니다.</li>
        </ol>
      ),
    },
  ];
      ...
      <div className="max-w-3xl mx-auto">
        <div className="mb-12">
          ...
          <p className="text-sm" style={{ fontFamily: "var(--font-noto-sans-kr)", color: "#5A4A3E" }}>
            시행일: 2026년 4월 25일 · 최종 개정일: 2026년 8월 3일
          </p>
        </div>
