...
const CODE_TEXT = { fontSize: "22px", fontWeight: 700, color: "#5C2A42", margin: 0, letterSpacing: "0.02em" } as const;
...
export function ReservationWaitlistedEmail({ name, showTitle, code, partySize }: Props) {
  ...
  return (
    <Html lang="ko">
      <Head />
      <Preview>{`「${showTitle}」 좌석 신청이 대기 접수되었습니다`}</Preview>
      <Body style={BODY}>
        <Container style={CONTAINER}>
          <Heading as="h1" style={GREETING}>{greeting}</Heading>
          <Text style={PARAGRAPH}>
            무대올림입니다. 「{showTitle}」은(는) 신청하신 정원({partySize}명)이 이미 다 찼습니다.
            대기자로 접수해두었으며, 자리가 나면 안내해 드립니다.
          </Text>
...
export default ReservationWaitlistedEmail;
