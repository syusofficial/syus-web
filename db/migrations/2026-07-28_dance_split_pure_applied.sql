-- ============================================================================
-- ██  폐기됨 (DEPRECATED) — 이 마이그레이션은 실행하지 마세요  ██
-- ============================================================================
-- 2026-07-28 오후, 사장님이 방침을 재변경했습니다:
--   최상위 장르는 "순수무용"/"실용무용"으로 나누지 않고 다시 "무용" 하나로
--   유지합니다. 하위 분류(순수무용/실용무용 및 그 아래 세부 장르)는 DB
--   genre 컬럼을 쪼개는 대신, 프런트엔드 NavMega의 "호버 드롭다운" UI로만
--   표현합니다 (src/lib/constants.ts의 GENRE_DETAIL_GROUPS 참고).
--
-- 즉, genre 컬럼 값은 계속 "무용" 그대로 두고, 세부 분류는 기존처럼
-- genre_detail 컬럼(한국무용/발레/현대무용/대중무용/댄스)으로만 구분합니다.
--
-- 이 파일은 아래 원래 목적(무용 → 순수무용/실용무용 UPDATE)을 절대 실행하지
-- 마세요. 실행된 적 없고(대상 0건 확인됨) DB에도 영향 없었습니다. 기록 보존
-- 목적으로만 파일을 남겨둡니다.
-- ============================================================================
